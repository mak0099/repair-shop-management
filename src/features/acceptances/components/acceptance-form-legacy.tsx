...
interface AcceptanceFormLegacyProps {
  initialData?: Acceptance | null
  onSuccess?: (data: Acceptance) => void
  isViewMode?: boolean
}

export function AcceptanceFormLegacy({ initialData, onSuccess, isViewMode: initialIsViewMode = false }: AcceptanceFormLegacyProps) {
  ...
  const form = useForm<any>({
    ...
    defaultValues: {
      ...
      currentStatus: initialData?.currentStatus || REPAIR_STATUSES.PENDING,
          ...
          <div className="relative grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="md:col-span-1 space-y-4">
              <div className="bg-card p-4 rounded-md shadow border space-y-3">
                ...
                <ModelSelectField name="modelId" control={control} brandId={brandId} required readOnly={isViewMode || !brandId} />
            ...
            <div className="md:col-span-1 space-y-4">
              <div className="bg-card p-4 rounded-md shadow border space-y-3">
                <DatePickerField control={control} name="acceptanceDate" label="Acceptance Date" required readOnly={isViewMode} />
            ...
            <div className="md:col-span-1 space-y-4">
              ...
              <div className="bg-card p-4 rounded-md shadow border space-y-4">
                <RadioGroupField control={control} required name="importantInformation" label="Important?" readOnly={isViewMode} options={[{ label: "Yes", value: "true" }, { label: "No", value: "false" }]} />
                <RadioGroupField control={control} required name="pinUnlock" label="Pin Unlock?" readOnly={isViewMode} options={[{ label: "Yes", value: "true" }, { label: "No", value: "false" }]} />
                {pinUnlock === "true" && (
                  <TextField control={control} name="pinUnlockNumber" label="PIN Code" required readOnly={isViewMode} />
                )}
                <RadioGroupField control={control} required name="urgent" label="Urgent?" readOnly={isViewMode} options={[{ label: "Yes", value: "true" }, { label: "No", value: "false" }]} />
                {urgent === "true" && (
                  <DatePickerField control={control} name="urgentDateTime" label="Deadline" required readOnly={isViewMode} showTime />
                )}
                <RadioGroupField control={control} required name="quote" label="Quote Needed?" readOnly={isViewMode} options={[{ label: "Yes", value: "true" }, { label: "No", value: "false" }]} />
              </div>
