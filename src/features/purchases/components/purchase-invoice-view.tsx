  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="max-w-4xl p-0 overflow-hidden h-[95vh] flex flex-col border-none shadow-2xl">
        ...
        <div className="p-10 print:p-0 overflow-y-auto flex-1 bg-background" id="printable-area">
                ...
                <tbody className="divide-y divide-border">
                  {purchase.items.map((item, idx) => (
                    <tr key={idx} className="align-top">
                      ...
                      <td className="py-5 px-2 text-right text-muted-foreground font-bold">{currency} {item.costPrice.toLocaleString()}</td>
                      <td className="py-5 px-4 text-right font-black text-foreground text-sm">{currency} {item.subtotal.toLocaleString()}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div className="flex justify-end pt-6">
              <div className="w-72 space-y-4">
                <div className="flex justify-between items-center text-[10px] font-black text-muted-foreground uppercase tracking-widest">
                  <span>Subtotal</span>
                  <span className="text-sm text-foreground">{currency} {purchase.subtotal.toLocaleString()}</span>
                </div>
                <div className="flex justify-between items-center text-[10px] font-black text-muted-foreground uppercase tracking-widest border-b pb-4">
                  ...
                  <span className="text-sm text-foreground">{currency} {purchase.paidAmount.toLocaleString()}</span>
                </div>
