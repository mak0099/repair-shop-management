    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {summaryCards.map((card, idx) => (
          <div key={idx} className={cn("p-4 rounded-2xl border border-dashed flex flex-col gap-1", card.bg)}>
      ...
      <div className="bg-slate-900 rounded-3xl p-6 text-white shadow-xl relative overflow-hidden">
        <div className="relative z-10 grid grid-cols-1 md:grid-cols-2 gap-8">
          <div>
            <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Total Revenue Generated</span>
          ...
          <div className="flex flex-col justify-end md:items-end">
            <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Expected Cash in Drawer</span>
            <h2 className="text-3xl font-black mt-1">৳{expectedInDrawer.toLocaleString()}</h2>
          </div>
        </div>
        
        {/* Background Decorative Icon */}
        <Landmark className="absolute -right-4 -bottom-4 h-32 w-32 text-white/5 rotate-12" />
      </div>
      ...
      {register.status === "CLOSED" && (
        <div className={cn(
          ...
          difference === 0 ? "bg-emerald-50 border-emerald-100 text-emerald-800" : 
          difference > 0 ? "bg-blue-50 border-blue-100 text-blue-800" : "bg-red-50 border-red-100 text-red-800"
        )}>
          <div className="flex items-center gap-3">
            {difference < 0 ? <ArrowDownCircle className="h-6 w-6 text-red-500" /> : <CheckCircleIcon difference={difference} />}
            <div>
...
function CheckCircleIcon({ difference }: { difference: number }) {
  return <ArrowUpCircle className={cn("h-6 w-6", difference === 0 ? "text-emerald-500" : "text-blue-500")} />
}